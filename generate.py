...
# Тематичні рубрики: slug -> (назва, ключові слова для класифікації).
# Кожен пост потрапляє в усі рубрики, чиї слова знайдено в тексті — це дає
# Google додаткові точки входу за довгими запитами («новини фронт сьогодні» тощо).
TAGS: dict[str, tuple[str, list[str]]] = {
    "front": ("Війна та фронт", [
        "обстріл", "атак", "дрон", "ракет", "фронт", "зсу", "бої", "боїв",
        "наступ", "удар", "ппо", "окупант", "ворог", "піхот", "танк", "артилер"]),
    "world": ("Світ і політика", [
        "сша", "євро", "трамп", "путін", "нато", "оон", "санкці", "переговор",
        "світов", "president", "байден", "росі", " рф ", "кремл", "дипломат"]),
    "economy": ("Економіка", [
        "курс", "гривн", "долар", "ціни", "ціна", "тариф", "бюджет", "економік",
        "пальне", "зарплат", "інфляц", "податк", "субсиді"]),
    "society": ("Суспільство", [
        "київ", "львів", "одес", "харків", "дніпр", "поліці", "суд", "школ",
        "медиц", "лікар", "загин", "постражд", "рятувальник", "пожеж", "дтп"]),
    "sport": ("Спорт", [
        "матч", "чемпіонат", "збірна", "футбол", " гол ", "олімп", "турнір",
        "перемог", "фінал", "клуб"]),
}
...
def assign_tags(text: str) -> list[str]:
    low = f" {text.lower()} "
    return [slug for slug, (_, kws) in TAGS.items() if any(k in low for k in kws)]


def _tag_nav(active: str = "") -> str:
    links = ['<a href="/">Головна</a>' if active else "<b>Головна</b>"]
    for slug, (name, _) in TAGS.items():
        if slug == active:
            links.append(f"<b>{name}</b>")
        else:
            links.append(f'<a href="{SITE_URL}tag/{slug}.html">{name}</a>')
    return '<nav style="margin:12px 0;font-size:.9em">' + " · ".join(links) + "</nav>"


def render_tag_page(slug: str, name: str, posts: list[dict]) -> str:
    title = f"{name} — Новини України сьогодні"
    desc = html.escape(f"{name}: свіжі новини України та світу. {SITE_DESC}")[:160]
    items = []
    for p in posts[:MAX_ON_PAGE]:
        items.append(f"""
  <article>
    <a href="{SITE_URL}posts/{p['id']}.html"><time datetime="{p['date_iso']}">{p['date_local']}</time></a>
    <p>{html.escape(p['text'])}</p>
    <a href="https://t.me/{CHANNEL}/{p['id']}" rel="noopener">Читати в Telegram →</a>
  </article>""")
...
<link rel="canonical" href="{SITE_URL}tag/{slug}.html">
<style>
  ...
  article{{border-bottom:1px solid #333;padding:14px 0}} time{{color:#888;font-size:.85em}}
  h1{{font-size:1.4em}}
</style>
</head>
<body>
...
<p><a class="cta" href="https://t.me/{CHANNEL}" rel="noopener">Всі новини — у Telegram-каналі</a></p>
</body>
</html>
"""
...
def render_sitemap(posts: list[dict], tag_slugs: list[str]) -> str:
    today = dt.date.today().isoformat()
    urls = [f"  <url><loc>{SITE_URL}</loc><lastmod>{today}</lastmod></url>"]
    for slug in tag_slugs:
        urls.append(f"  <url><loc>{SITE_URL}tag/{slug}.html</loc><lastmod>{today}</lastmod></url>")
    for p in posts:
        urls.append(
...
def main() -> None:
    ...
    tag_map: dict[str, list[dict]] = {slug: [] for slug in TAGS}
    for p in posts:
        for slug in assign_tags(p["text"]):
            tag_map[slug].append(p)
    active_tags = []
    for slug, (name, _) in TAGS.items():
        if tag_map[slug]:
            (tag_dir / f"{slug}.html").write_text(
                render_tag_page(slug, name, tag_map[slug]), encoding="utf-8")
            active_tags.append(slug)

    (BASE / "sitemap.xml").write_text(render_sitemap(posts, active_tags), encoding="utf-8")
    key = ensure_indexnow_key(state)
    save_state(state)
    print(f"Сайт згенеровано: {len(posts)} сторінок, рубрик: {len(active_tags)}, нових: {len(new_urls)}")
    if new_urls:
        ping_indexnow(key, [SITE_URL] + new_urls + [f"{SITE_URL}tag/{s}.html" for s in active_tags])
